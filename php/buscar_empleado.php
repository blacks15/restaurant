<?php 
  ...
  if($post['search'] == 'true'){
    ...
    $search['where'] = elements(array('nombre_empleado','apellidos','ciudad','estado'),$_REQUEST);
  ...
  $query = mysql_query("select count(*) as t from empleados".$se);
  if(!$query){
    echo mysql_error();
  } 
  ...
  while($row = mysql_fetch_object($query)){
    ...
    $result[$i]['cell'] = array($row->matricula,$row->nombre_empleado,$row->apellidos,
    $row->direccion,$row->ciudad,$row->estado,$row->telefono,$row->celular,
    $row->sueldo,$row->tipo,$row->status);
    $i++;
  }     
